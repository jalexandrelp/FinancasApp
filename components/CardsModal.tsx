...
export default function CardsModal({ visible, onClose }: CardsModalProps) {
  const themeCtx = useContext(ThemeContext) as any;
  const ctx = useContext(FinanceContext);
  if (!themeCtx || !ctx) return null;

  const { theme } = themeCtx;
  const { cards, addCard, updateCard, deleteCard } = ctx;

  const [editing, setEditing] = useState<Card>({ id: '', name: '' });
  ...
  const handleSave = () => {
    ...
    if (editing.id) {
      updateCard({ id: editing.id, name });
    } else {
      addCard({ id: Date.now().toString(), name });
    }
    setEditing({ id: '', name: '' });
  };

  const handleEdit = (item: Card) => setEditing(item);
  const handleDelete = (id: string) => deleteCard(id);
    ...
    <Modal visible={visible} transparent animationType="slide" onRequestClose={onClose}>
      <SafeAreaView style={[styles.overlay, { backgroundColor: 'rgba(0,0,0,0.3)' }]}>
        <View style={[styles.modal, { backgroundColor: theme.card }]}>
          <Text style={[styles.title, { color: theme.text }]}>Meus Cartões</Text>
          ...
          <Pressable
            ...
            onPress={() => { onClose(); setEditing({ id: '', name: '' }); }}
          >
